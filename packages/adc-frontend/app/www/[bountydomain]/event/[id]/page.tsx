...
export default function EventPage({ params }: IPageProps) {
	...
	const event = events.find((data) => data.id === Number(id));
				...
				>
					...
					<div className="relative order-2 flex flex-1 flex-col gap-5 border-l px-[70px]">
						<h1 className="text-center font-extrabold text-[18px]/[26px]">
							{event.name} {event.submitterName ? `(${event.submitterName})` : ""}
						</h1>
						...
						<TabsContent value="tab-2">
							<div className="grid grid-cols-3 gap-[25px]">
								{["contract", "Document", "Sales"].map((item) => (
									<div
										key={item}
										className="flex w-full items-start bg-[#F3F3F3] px-3.5 py-2.5"
									>
